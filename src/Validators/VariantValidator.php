		...
		if (!empty($validationData)) {
			foreach ($validationData[Product::KEY_VARIANTS] as $index => $variantData) {
				try {
					...
					// variant longdesc
					if ($this->validateLength($variantData[Variant::KEY_LONGDESC], self::MAX_LONGDESC_LENGTH) === false) {
						throw $this->generateThrow(sprintf(ValidatorException::MSG_INVALID_VALUE_MAX_LENGTH, Variant::KEY_LONGDESC, $variantData[Variant::KEY_LONGDESC], self::MAX_LONGDESC_LENGTH));
					}
					// variant priority
